...
class ovsdpdk::postinstall_ovs_dpdk (
  ...
  if $compute == 'True' {
    ...
    exec { "remove_old_agent":
      command => "${plugin_dir}/files/remove_agent.sh $adminrc_user $adminrc_password $adminrc_tenant $adminrc_hostname $fqdn",
      user    => 'root',
      require => Exec['restart_ovs'],
    }
    ...
    exec {'restart_nova_compute':
      ...
      user    => root,
      require => [ Exec['remove_old_agent'], Exec['libvirtd_disable_tls'], Service['libvirtd'] ],
    }
  }
