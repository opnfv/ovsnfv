...
if ! rpm -q rdo-release > /dev/null && [ "$1" != "-master" ]; then
    ...
    sudo curl -o /etc/yum.repos.d/delorean.repo https://trunk.rdoproject.org/centos7/consistent/delorean.repo
    ...
    sudo yum -y install yum-plugin-priorities
    sudo yum-config-manager --disable openstack-${RDO_RELEASE}
    sudo curl -o /etc/yum.repos.d/delorean.repo http://trunk.rdoproject.org/centos7/consistent/delorean.repo
    sudo curl -o /etc/yum.repos.d/delorean-deps.repo http://trunk.rdoproject.org/centos7-liberty/delorean-deps.repo
    sudo rm -f /etc/yum.repos.d/delorean-current.repo
fi
...
ssh -T ${SSH_OPTIONS[@]} "root@$UNDERCLOUD" <<EOI
    ...
    curl -o /etc/yum.repos.d/delorean.repo http://trunk.rdoproject.org/centos7-liberty/consistent/delorean.repo
